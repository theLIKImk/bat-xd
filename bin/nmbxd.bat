...
:login
	...
	call :curl-get https://www.nmbxd1.com/Member/User/Index/verify.html --output "%NA_TMP%v.png"
	start "" "%NA_TMP%v.png"
	echo.  ^|
	echo.  ^| 请输入已打开的图像验证码，如果未打开请手动打开文件路径：
	echo.  ^| %NA_TMP%v.png
	echo.  ^|
	set /p v_c=验证码:
	...
	if "!NA_cookie_num!"=="0" echo.  ^| x 饼干未检测到，请检测账户是否登录错误！ & set NA_EXIT=10& goto out_login
...
:cookie
	...
	echo.# 饼干文件，自动生成！ >>"%~dp0/cookies.txt"
	echo.# 生成时间：%date% %time%>>"%~dp0/cookies.txt"
	echo.>>"%~dp0/cookies.txt"
